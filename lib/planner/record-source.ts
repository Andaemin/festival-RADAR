...
export async function loadPlannerCorpus(): Promise<PlannerCorpus> {
    ...
    for (const row of rows) {
        ...
        if (!existing) {
            merged.set(key, {
                ...
                totalBudgetKrw: row.budgetTotalKrw !== null ? Number(row.budgetTotalKrw) : null,
                visitors: row.visitorTotalPersons,
                keywords: [],
            });
            continue;
        }
        ...
        existing.totalBudgetKrw ??= row.budgetTotalKrw !== null ? Number(row.budgetTotalKrw) : null;
        existing.visitors ??= row.visitorTotalPersons;
    }
    ...
    const corpus: PlannerCorpus = { datasetYear, datasetYearRange, records, vocabulary, districtNames };
